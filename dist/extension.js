...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('webbot.chatView', provider, {
        webviewOptions: {
            retainContextWhenHidden: true
        }
    }));
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('webbot.openChat', () => {
        vscode.commands.executeCommand('workbench.view.extension.webbot-sidebar');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('webbot.newChat', () => {
        provider.newChat();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('webbot.clearHistory', () => {
        provider.clearHistory();
    }));
    // Context menu command - send selected code to chat
    context.subscriptions.push(vscode.commands.registerCommand('webbot.askAboutCode', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('webbot.explainFile', () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
...
class ChatViewProvider {
    ...
    async handleSendMessage(userMessage) {
        ...
        const apiUrl = config.get('apiUrl') || 'http://localhost:8787/api/chat/stream';
        // Add user message
        const userMsg = {
            id: this.makeId(),
            role: 'user',
    ...
    async handleAttachActiveEditor() {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
...
ChatViewProvider.viewType = 'webbot.chatView';
